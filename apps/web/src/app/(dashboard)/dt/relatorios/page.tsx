...
export default function RelatoriosPage() {
  ...
  function getFieldStats(sectionId: string, fieldId: string): StatsData[] {
    ...
    return Object.entries(counts)
      .map(([label, value]) => ({ label, value, percentage: total > 0 ? Math.round((value / total) * 100) : 0 }))
  ...
  function exportCharacterizationPDF() {
    ...
    doc.setFontSize(9);
    doc.text(`${classroom?.schools?.name} — ${classroom?.year_grade} ${classroom?.label} (${classroom?.shift})`, 14, 21);
    ...
    autoTable(doc, { head: [headers], body, startY: 25, styles: { fontSize: 7 }, headStyles: { fillColor: [34, 139, 34] } });
  ...
  function exportStatsPDF() {
    ...
    doc.setFontSize(9);
    doc.text(`${classroom?.schools?.name} — ${classroom?.year_grade} ${classroom?.label}`, 14, 21);
    ...
    for (const section of sections) {
      ...
      if (section.data.length > 0) {
        autoTable(doc, { head: [['Resposta', 'Qtd', '%']], body: section.data.map((d) => [d.label, d.value.toString(), `${d.percentage}%`]), startY: y, styles: { fontSize: 8 }, headStyles: { fillColor: [34, 139, 34] }, margin: { left: 14 } });
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="caracterizacao">
        <TabsList>
          <TabsTrigger value="caracterizacao"><FileText className="mr-2 h-4 w-4" />Caracterização</TabsTrigger>
          <TabsTrigger value="estatisticas"><BarChart3 className="mr-2 h-4 w-4" />Dados Estatísticos</TabsTrigger>
          <TabsTrigger value="disciplinas"><PieChart className="mr-2 h-4 w-4" />Por Disciplina</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="caracterizacao">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={exportCharacterizationPDF}><Download className="mr-2 h-4 w-4" />PDF</Button>
            </CardHeader>
            <CardContent>
                    ...
                    {characterization.map((r) => {
                      ...
                      return (
                        <TableRow key={r.student_id}>
                          ...
                          <TableCell>{r.birthdate ? new Date(r.birthdate).toLocaleDateString('pt-BR') : '—'}</TableCell>
        ...
        <TabsContent value="estatisticas">
          <div className="space-y-4">
            <div className="flex justify-end">
              <Button variant="outline" size="sm" onClick={exportStatsPDF}><Download className="mr-2 h-4 w-4" />PDF Completo</Button>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <Card><CardContent className="pt-6 text-center"><div className="text-3xl font-bold">{generalStats.total}</div><p className="text-sm text-muted-foreground">Total de Alunos</p></CardContent></Card>
              <Card><CardContent className="pt-6 text-center"><div className="text-3xl font-bold text-green-600">{generalStats.bio_completed}</div><p className="text-sm text-muted-foreground">Fichas Completas</p></CardContent></Card>
              <Card><CardContent className="pt-6 text-center"><div className="text-3xl font-bold text-yellow-600">{generalStats.bio_pending}</div><p className="text-sm text-muted-foreground">Fichas Pendentes</p></CardContent></Card>
            </div>
            {[
              { title: 'Tipo de Moradia', data: getFieldStats('familia', 'tipo_moradia') },
              ...
              <Card key={chart.title}>
                ...
                <CardContent>
                  {chart.data.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados preenchidos.</p> : (
                    <div className="space-y-3">
                      {chart.data.map((item) => (
                        <div key={item.label}>
                          <div className="flex justify-between text-sm mb-1"><span>{item.label}</span><span className="text-muted-foreground">{item.value} ({item.percentage}%)</span></div>
                          <div className="h-4 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full transition-all" style={{ width: `${item.percentage}%` }} /></div>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <TabsContent value="disciplinas">
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><span className="text-green-600">★</span> Disciplinas Preferidas</CardTitle></CardHeader>
              <CardContent>
                {discPreferidas.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados.</p> : (
                  <div className="space-y-3">
                    {discPreferidas.map((item, idx) => (
                      <div key={item.label} className="flex items-center gap-3">
                        <span className="text-lg font-bold text-muted-foreground w-6">{idx + 1}</span>
                        <div className="flex-1">
                          <div className="flex justify-between text-sm mb-1"><span className="font-medium">{item.label}</span><span>{item.value} ({item.percentage}%)</span></div>
                          <div className="h-3 bg-muted rounded-full overflow-hidden"><div className="h-full bg-green-500 rounded-full" style={{ width: `${item.percentage}%` }} /></div>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Card>
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><span className="text-red-500">!</span> Disciplinas com Dificuldade</CardTitle></CardHeader>
              <CardContent>
                {discDificuldade.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados.</p> : (
                  <div className="space-y-3">
                    {discDificuldade.map((item, idx) => (
                      <div key={item.label} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-sm mb-1"><span className="font-medium">{item.label}</span><span>{item.value} ({item.percentage}%)</span></div>
                          <div className="h-3 bg-muted rounded-full overflow-hidden"><div className="h-full bg-red-500 rounded-full" style={{ width: `${item.percentage}%` }} /></div>
                        </div>
                      </div>
                    ))}
                  </div>
